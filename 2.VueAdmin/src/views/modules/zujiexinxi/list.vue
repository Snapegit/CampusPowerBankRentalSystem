<template>
	<div class="main-content" :style='{"color":"#666","padding":"0px 30px 30px","fontSize":"14px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","margin":"0 0 20px","borderRadius":"0px","textAlign":"left","background":"none","display":"block","width":"100%","fontSize":"inherit","order":"2"}' >
					<div :style='{"margin":"0 0px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">充电宝名称</label>
						<el-input v-model="searchForm.chongdianbaomingcheng" placeholder="充电宝名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 0px 0 0","fontSize":"inherit","display":"inline-block"}'>
				...
				<el-row class="actions" :style='{"margin":"0px 0 20px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit","order":"1"}'>
					<el-button class="add" v-if="isAuth('zujiexinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('zujiexinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"0px solid #eee","width":"100%","padding":"0","margin":"0px 0 0","borderRadius":"12px","background":"rgba(255,255,255,.9)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<!-- $column.hiden -->
					<el-table-column :resizable='true' :sortable='true'  
						prop="chongdianbaomingcheng"
						label="充电宝名称">
						<template slot-scope="scope">
							{{scope.row.chongdianbaomingcheng}}
						</template>
					</el-table-column>
					<!-- chongdianbaoleixing -->
					<!-- $column.hiden -->
					<el-table-column :resizable='true' :sortable='true'  
						prop="chongdianbaoleixing"
						label="充电宝类型">
						<template slot-scope="scope">
							{{scope.row.chongdianbaoleixing}}
						</template>
					</el-table-column>
					<!-- pinpai -->
					<!-- $column.hiden -->
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('zujiexinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('zujiexinxi','归还')" @click="guihaixinxiCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								归还
							</el-button>
							<el-button class="btn8" v-if="isAuth('zujiexinxi','报修')" @click="chongdianbaoguzhangCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								报修
							</el-button>
							<el-button class="edit" v-if=" isAuth('zujiexinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('zujiexinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		<guihaixinxi-cross-add-or-update v-if="guihaixinxiCrossAddOrUpdateFlag" :parent="this" ref="guihaixinxiCrossaddOrUpdate"></guihaixinxi-cross-add-or-update>
		<chongdianbaoguzhang-cross-add-or-update v-if="chongdianbaoguzhangCrossAddOrUpdateFlag" :parent="this" ref="chongdianbaoguzhangCrossaddOrUpdate"></chongdianbaoguzhang-cross-add-or-update>
...
import guihaixinxiCrossAddOrUpdate from "../guihaixinxi/add-or-update";
import chongdianbaoguzhangCrossAddOrUpdate from "../chongdianbaoguzhang/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				guihaixinxiCrossAddOrUpdateFlag: false,
				chongdianbaoguzhangCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			guihaixinxiCrossAddOrUpdate,
			chongdianbaoguzhangCrossAddOrUpdate,
		},
		methods: {
			contentStyleChange() {
			...
			guihaixinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.guihaixinxiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','zujiexinxi');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.guihaixinxiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.guihaixinxiCrossaddOrUpdate.init(row.id,type);
				});
			},
			chongdianbaoguzhangCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.chongdianbaoguzhangCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','zujiexinxi');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.chongdianbaoguzhangCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.chongdianbaoguzhangCrossaddOrUpdate.init(row.id,type);
				});
			},
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.chongdianbaomingcheng!='' && this.searchForm.chongdianbaomingcheng!=undefined){
            params['chongdianbaomingcheng'] = '%' + this.searchForm.chongdianbaomingcheng + '%'
          }
	...
	zujiexinxistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'zujiexinxi/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
