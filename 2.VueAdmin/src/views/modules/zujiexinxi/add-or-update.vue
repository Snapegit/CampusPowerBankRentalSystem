<template>
	<div class="addEdit-block" :style='{"padding":"0px 0px 30px","fontSize":"14px","color":"#000","background":"none"}'>
		...
		>
			<template >
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-if="type!='info'" label="租借编号" prop="zujiebianhao">
					<el-input v-model="ruleForm.zujiebianhao" placeholder="租借编号" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-else-if="ruleForm.zujiebianhao" label="租借编号" prop="zujiebianhao">
					<el-input v-model="ruleForm.zujiebianhao" placeholder="租借编号" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="date" v-if="type!='info'" label="租借时间" prop="zujieshijian">
					<el-date-picker
						value-format="yyyy-MM-dd HH:mm:ss"
						v-model="ruleForm.zujieshijian" 
						type="datetime"
						:readonly="ro.zujieshijian"
						placeholder="租借时间"
					></el-date-picker>
				</el-form-item>
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-else-if="ruleForm.zujieshijian" label="租借时间" prop="zujieshijian">
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-if="type!='info'"  label="电话号码" prop="dianhuahaoma">
					<el-input v-model="ruleForm.dianhuahaoma" placeholder="电话号码" clearable  :readonly="ro.dianhuahaoma"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="电话号码" prop="dianhuahaoma">
...
export default {
	...
	created() {
		this.ruleForm.zujieshijian = this.getCurDateTime()
	},
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						if(o=='zujiebianhao'){
							this.ruleForm.zujiebianhao = obj[o];
							this.ro.zujiebianhao = true;
							continue;
						}
    ...
    // 提交
    onSubmit() {
		if(this.ruleForm.zujiebianhao) {
			this.ruleForm.zujiebianhao = String(this.ruleForm.zujiebianhao)
		}
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `zujiexinxi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.zujiexinxiCrossAddOrUpdateFlag = false;
											this.parent.search();
