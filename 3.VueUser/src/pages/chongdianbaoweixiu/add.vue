...
<div :style='{"width":"80%","padding":"20px","margin":"10px auto","position":"relative","background":"#fff"}'>
    ...
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="报障编号" prop="baozhangbianhao">
            <el-input v-model="ruleForm.baozhangbianhao" 
                placeholder="报障编号" clearable :disabled=" false  ||ro.baozhangbianhao"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="维修时间" prop="weixiushijian">
              <el-date-picker
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="人员账号" prop="renyuanzhanghao">
            <el-input v-model="ruleForm.renyuanzhanghao" 
                placeholder="人员账号" clearable :disabled=" false  ||ro.renyuanzhanghao"></el-input>
          </el-form-item>
...
<script>
  export default {
      ...
      init(type) {
        ...
        this.$http.get(this.userTableName + '/session', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            ...
            if((json.renyuanzhanghao!=''&&json.renyuanzhanghao) || json.renyuanzhanghao==0){
                this.ruleForm.renyuanzhanghao = json.renyuanzhanghao
            }
            if((json.renyuanxingming!=''&&json.renyuanxingming) || json.renyuanxingming==0){
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`chongdianbaoweixiu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('chongdianbaoweixiu/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`chongdianbaoweixiu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
