<template>
	<div class="main-content" :style='{"color":"#666","padding":"0px 30px 30px","fontSize":"14px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"margin":"0px 0 20px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit","order":"1"}'>
					<el-button class="add" v-if="isAuth('chongdianbaoguzhang','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('chongdianbaoguzhang','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('chongdianbaoguzhang','审核')" :disabled="dataListSelections.length?false:true" type="success" @click="shBatchDialog()">
			...
			<div :style='{"border":"0px solid #eee","width":"100%","padding":"0","margin":"0px 0 0","borderRadius":"12px","background":"rgba(255,255,255,.9)"}'>
				<el-table class="tables"
					...
					:border='true'
					v-if="isAuth('chongdianbaoguzhang','查看')"
					:data="dataList"
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="baozhangbianhao"
						label="报障编号">
						<template slot-scope="scope">
							{{scope.row.baozhangbianhao}}
						</template>
					</el-table-column>
					...
					<!-- $column.hiden -->
					<el-table-column :resizable='true' :sortable='true'  
						prop="baozhangshijian"
						label="报障时间">
						<template slot-scope="scope">
							{{scope.row.baozhangshijian}}
						</template>
					</el-table-column>
					<!-- yonghuzhanghao -->
					<!-- $column.hiden -->
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('chongdianbaoguzhang','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('chongdianbaoguzhang','维修')" @click="chongdianbaoweixiuCrossAddOrUpdateHandler(scope.row,'cross','是','','weixiuzhuangtai','已维修','已维修,待维修'.split(',')[0])" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								维修
							</el-button>
							<el-button class="edit" v-if=" isAuth('chongdianbaoguzhang','修改')  && scope.row.sfsh=='待审核' " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('chongdianbaoguzhang','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		<chongdianbaoweixiu-cross-add-or-update v-if="chongdianbaoweixiuCrossAddOrUpdateFlag" :parent="this" ref="chongdianbaoweixiuCrossaddOrUpdate"></chongdianbaoweixiu-cross-add-or-update>
...
import chongdianbaoweixiuCrossAddOrUpdate from "../chongdianbaoweixiu/add-or-update";
	export default {
		data() {
			return {
				searchForm: {
				...
				chongdianbaoweixiuCrossAddOrUpdateFlag: false,
		...
		components: {
			AddOrUpdate,
			chongdianbaoweixiuCrossAddOrUpdate,
		},
		methods: {
			contentStyleChange() {
			...
			chongdianbaoweixiuCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.chongdianbaoweixiuCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','chongdianbaoguzhang');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.chongdianbaoweixiuCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.chongdianbaoweixiuCrossaddOrUpdate.init(row.id,type);
				});
			},
    ...
    shBatchHandler(){
      this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
        ...
        this.$http({
          url: "chongdianbaoguzhang/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
          method: "post",
          data: this.batchIds
        }).then(({ data }) => {
          if (data && data.code === 0) {
	...
	chongdianbaoguzhangstatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'chongdianbaoguzhang/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
    ...
    deleteHandler(id ) {
      ...
      this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
        ...
        this.$http({
          url: "chongdianbaoguzhang/delete",
          method: "post",
          data: ids
        }).then(({ data }) => {
          if (data && data.code === 0) {
