...
<div :style='{"width":"80%","padding":"20px","margin":"10px auto","position":"relative","background":"#fff"}'>
    <el-form
      ...
      label-width="100px"
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="报障编号" prop="baozhangbianhao">
              <el-input v-model="ruleForm.baozhangbianhao" placeholder="报障编号" disabled></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="充电宝类型" prop="chongdianbaoleixing">
            <el-input v-model="ruleForm.chongdianbaoleixing" 
                placeholder="充电宝类型" clearable :disabled=" false  ||ro.chongdianbaoleixing"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}'  label="维修状态" prop="weixiuzhuangtai">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="报障时间" prop="baozhangshijian">
              <el-date-picker
				  :disabled=" false  ||ro.baozhangshijian"
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="人员账号" prop="renyuanzhanghao">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="人员姓名" prop="renyuanxingming">
            <el-input v-model="ruleForm.renyuanxingming" 
                placeholder="人员姓名" clearable :disabled=" false  ||ro.renyuanxingming"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="故障内容" prop="guzhangneirong">
            <editor 
  ...
  export default {
    ...
    methods: {
      ...
      init(type) {
        ...
        this.$http.get('option/weixiurenyuan/renyuanzhanghao', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            this.renyuanzhanghaoOptions = res.data.data;
          }
        });
      ...
      renyuanzhanghaoChange () {
        this.$http.get('follow/weixiurenyuan/renyuanzhanghao?columnValue=' + this.ruleForm.renyuanzhanghao, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            if(res.data.data.renyuanxingming){
              this.ruleForm.renyuanxingming = res.data.data.renyuanxingming
            }
          }
        });
      },
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`chongdianbaoguzhang/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('chongdianbaoguzhang/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`chongdianbaoguzhang/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
