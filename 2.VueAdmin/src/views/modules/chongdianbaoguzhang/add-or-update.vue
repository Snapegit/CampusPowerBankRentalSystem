	...
	<div class="addEdit-block" :style='{"padding":"0px 0px 30px","fontSize":"14px","color":"#000","background":"none"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-else-if="ruleForm.baozhangbianhao" label="报障编号" prop="baozhangbianhao">
					<el-input v-model="ruleForm.baozhangbianhao" placeholder="报障编号" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="select" v-if="type!='info'"  label="维修状态" prop="weixiuzhuangtai">
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="维修状态" prop="weixiuzhuangtai">
					<el-input v-model="ruleForm.weixiuzhuangtai"
						placeholder="维修状态" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="date" v-if="type!='info'" label="报障时间" prop="baozhangshijian">
					<el-date-picker
						format="yyyy 年 MM 月 dd 日"
						value-format="yyyy-MM-dd"
						v-model="ruleForm.baozhangshijian" 
						type="date"
						:readonly="ro.baozhangshijian"
						placeholder="报障时间"
					></el-date-picker> 
				</el-form-item>
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-else-if="ruleForm.baozhangshijian" label="报障时间" prop="baozhangshijian">
					<el-input v-model="ruleForm.baozhangshijian" placeholder="报障时间" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="select" v-if="type!='info'" label="人员账号" prop="renyuanzhanghao">
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-else-if="ruleForm.renyuanzhanghao" label="人员账号" prop="renyuanzhanghao">
				...
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-if="type!='info'"  label="故障内容" prop="guzhangneirong">
					<editor 
						style="min-width: 200px; max-width: 600px;"
						v-model="ruleForm.guzhangneirong" 
						class="editor" 
						action="file/upload">
					</editor>
				</el-form-item>
				<el-form-item :style='{"width":"48%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else-if="ruleForm.guzhangneirong" label="故障内容" prop="guzhangneirong">
                    <span :style='{"padding":"0 20px","color":"inherit","background":"#fff","display":"inline-block","width":"80%","fontSize":"14px","lineHeight":"40px","fontWeight":"500"}' v-html="ruleForm.guzhangneirong"></span>
                </el-form-item>
...
export default {
	data() {
		...
		return {
			...
			ro:{
				...
				guzhangneirong : false,
				tupian : false,
				weixiuzhuangtai : false,
				baozhangshijian : false,
	...
	created() {
		this.ruleForm.baozhangshijian = this.getCurDate()
	},
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='guzhangneirong'){
							this.ruleForm.guzhangneirong = obj[o];
							this.ro.guzhangneirong = true;
							continue;
						}
						if(o=='tupian'){
						...
						if(o=='baozhangshijian'){
							this.ruleForm.baozhangshijian = obj[o];
							this.ro.baozhangshijian = true;
							continue;
						}
						if(o=='yonghuzhanghao'){
            ...
            this.$http({
				url: `option/weixiurenyuan/renyuanzhanghao`,
				method: "get"
            }).then(({ data }) => {
				if (data && data.code === 0) {
			...
			renyuanzhanghaoChange () {
				this.$http({
					url: `follow/weixiurenyuan/renyuanzhanghao?columnValue=`+ this.ruleForm.renyuanzhanghao,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
    ...
    info(id) {
      this.$http({
        url: `chongdianbaoguzhang/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
        ...
        this.ruleForm.guzhangneirong = this.ruleForm.guzhangneirong.replace(reg,'../../../ssmg7r00u50/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    // 提交
    onSubmit() {
		if(this.ruleForm.baozhangbianhao) {
			this.ruleForm.baozhangbianhao = String(this.ruleForm.baozhangbianhao)
		}
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `chongdianbaoguzhang/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.chongdianbaoguzhangCrossAddOrUpdateFlag = false;
											this.parent.search();
