...
<div :style='{"width":"80%","padding":"20px","margin":"10px auto","position":"relative","background":"#fff"}'>
    <el-form
      ...
      label-width="100px"
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="归还编号" prop="guihaibianhao">
              <el-input v-model="ruleForm.guihaibianhao" placeholder="归还编号" disabled></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="租借时间" prop="zujieshijian">
            <el-input v-model="ruleForm.zujieshijian" 
                placeholder="租借时间" clearable :disabled=" false  ||ro.zujieshijian"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="时长" prop="shizhang">
              <el-input v-model="shizhang" placeholder="时长" disabled></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="订单状态" prop="dingdanzhuangtai">
            <el-input v-model="ruleForm.dingdanzhuangtai" 
                placeholder="订单状态" clearable :disabled=" false  ||ro.dingdanzhuangtai"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="总金额" prop="jine">
              <el-input v-model="jine" placeholder="总金额" disabled></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="金额" prop="yonghuyue">
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`guihaixinxi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
      ...
      onSubmit() {
			...
			if(this.ruleForm.jine>this.ruleForm.yonghuyue){
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('guihaixinxi/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`guihaixinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
